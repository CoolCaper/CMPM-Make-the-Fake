...
class Level1 extends Phaser.Scene {
  ...
  create() {
    ...
    this.Ammo2 = this.physics.add
      .sprite(50, 50, "ammo")
      .body.setAllowGravity(false);
    this.Ammo3 = this.physics.add
      .sprite(75, 50, "ammo")
      .body.setAllowGravity(false);
    this.All_Ammo = [this.Ammo, this.Ammo2, this.Ammo3];

    this.ss = this.physics.add
      .sprite(game.config.width / 2, game.config.height / 4.8 - 50, "ss")
      .body.setAllowGravity(false);
    ...
    this.enemy2 = new Enemy(
      this,
      game.config.width / 6 - 50,
      game.config.height / 2,
      "test"
    );
    this.enemy3 = new Enemy(
      this,
      game.config.width / 1.2 - 120,
      game.config.height / 2,
      "test"
    );
    this.enemy4 = new Enemy(
      this,
      game.config.width / 1.1 - 100,
    ...
    this.instructions = this.input.keyboard.addKey(
      Phaser.Input.Keyboard.KeyCodes.I
    );
    this.restart = this.input.keyboard.addKey(Phaser.Input.Keyboard.KeyCodes.R);
    this.i_visible = false; //instructions toggle
    this.instruct = this.add
      .text(
        50,
        100,
      ...
      .setVisible(false);
    this.instruct2 = this.add
      .text(
        50,
        180,
        "Press F to shoot! (But beware! You have limited\nammo! Check the top left corner to see how much\nyou have left)",
        this.instruct_config
      )
      .setVisible(false);
    this.instruct3 = this.add
      .text(
        50,
        280,
        "Press space to jump, avoid enemies, and most\nimportantly, collect the Scooby Snacks\n(the box with the s on it)!",
        this.instruct_config
      )
      .setVisible(false);
    this.instruct4 = this.add
      .text(
        50,
        380,
        "Press 1 to start over from level 1\nFinally, Press I to make this text go away!",
        this.instruct_config
      )
      .setVisible(false);

    this.i_toggle = this.add.text(
      550,
      575,
      "Press I to toggle the instructions!",
      this.instruct_toggle
    );
    this.ammo_count = 1;
